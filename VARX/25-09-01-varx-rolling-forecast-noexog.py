...
window=1
test_size=0.2
order = (1, 0) # 20-day forecast were trained with (3, 0) # next-day forecasts were trained with (1, 0)

# load data
csv_path = Path("~/Dropbox/elvas-thesis/varx/data/dataset_stationary_with_us10y_final.csv").expanduser()
img_path = Path(f"~/Dropbox/elvas-thesis/varx/results_win{window}_testsize{test_size}_p{order[0]}_q{order[1]}_exog{use_exog}").expanduser()
...
if use_exog:
    ...
    model = VARMAX(endog=endog_train, order=order)
...
for i in np.arange(len(endog_test.index)):
    ...
    pm  = res_rolling.get_forecast(steps=window).predicted_mean  # <-- label-free
    yhat_t = pm.iloc[window-1]
    yhat_t.name = t_end                             # <-- stamp with the date
    preds.append(yhat_t)
    # append values to avoid index-extension checks
    res_rolling = res_rolling.append(endog_test.loc[t_start:t_end].values, refit=False)
    # if use_exog:
