...
if isbinary:
    ...
    outdir = Path(f"plots_xgb_block_bin/win{window}_p{p}_testsize{test_size}"); outdir.mkdir(exist_ok=True, parents=True)
else:
    endog_cols = ["ENEL_logret","NKT_logret","VWS_logret"]
    outdir = Path(f"plots_xgb_block/win{window}_p{p}_testsize{test_size}"); outdir.mkdir(exist_ok=True, parents=True)
...
# === 50/50 chronological split (on the aligned, dropna’d frame) ===
